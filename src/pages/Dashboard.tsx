...
function Dashboard() {
            ...
            {menuItems.map((item) => (
              <button
                ...
                className={`flex flex-col items-center gap-1 px-4 py-2 rounded-lg transition-colors ${
                  activeTab === item.id ? 'text-indigo-600' : 'text-gray-500 hover:text-gray-700'
                }`}
              >
                <div className={activeTab === item.id ? 'text-indigo-600' : 'text-gray-500'}>
                  {item.icon}
                </div>
                <span className={`text-xs font-medium ${activeTab === item.id ? 'text-indigo-600' : 'text-gray-500'}`}>
                  {item.label}
                </span>
              </button>
            ))}
  ...
  return (
    <div className="min-h-screen bg-gray-100">
      {/* Overlay pour fermer la sidebar sur tablette */}
      ...
      {/* Sidebar */}
      <aside className={`fixed top-0 left-0 z-50 h-screen transition-all duration-300 flex flex-col shadow-xl bg-blue-600
        ${sidebarOpen ? 'w-72' : (isTablet ? 'w-0 overflow-hidden' : 'w-16')}
      `}>
        {/* Logo */}
        <div className={`p-4 border-b border-white/20 ${!sidebarOpen && isTablet ? 'hidden' : ''}`}>
          <div className={`flex items-center gap-3 ${!sidebarOpen && !isTablet ? 'justify-center' : ''}`}>
        ...
        {/* Navigation */}
        <nav className={`flex-1 mt-8 overflow-y-auto ${!sidebarOpen && isTablet && 'hidden'}`}>
          {menuItems.map((item) => (
            <button
              key={item.id}
              onClick={() => {
              ...
              } ${!sidebarOpen && !isTablet ? 'justify-center px-3' : ''}`}
            >
              {item.icon}
              {(sidebarOpen || isTablet) && <span className="text-sm font-medium">{item.label}</span>}
            </button>
          ))}
        </nav>

        {/* Footer Sidebar */}
        <div className={`p-4 border-t border-white/20 ${!sidebarOpen && isTablet && 'hidden'}`}>
          <div className="bg-white/10 rounded-xl p-4">
            <div className={`flex items-center gap-3 mb-3 pb-3 border-b border-white/20 ${!sidebarOpen && !isTablet ? 'flex-col' : ''}`}>
              ...
              {(sidebarOpen || isTablet) && (
                <button onClick={handleLogout} className="text-white/70 hover:text-white flex-shrink-0" title="Déconnexion">
                  <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1" />
                  </svg>
                </button>
      ...
      {/* Main Content */}
      <div className={`transition-all duration-300 ${sidebarOpen ? (isTablet ? 'ml-0' : 'ml-72') : (isTablet ? 'ml-0' : 'ml-16')}`}>
        <header className="bg-white shadow-sm border-b sticky top-0 z-30">
          <div className="px-4 sm:px-6 py-4">
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-3">
                {(isTablet || (!sidebarOpen && !isTablet)) && (
                  <button
              ...
              <div className="relative">
                <input
                  ...
                  className="pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-500 focus:border-transparent w-48 sm:w-64"
                />
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
                <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 hover:shadow-md transition-shadow">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-gray-500 text-xs sm:text-sm">Total Factures</p>
                ...
                <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 hover:shadow-md transition-shadow">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-gray-500 text-xs sm:text-sm">Montant Total</p>
                ...
                <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 hover:shadow-md transition-shadow">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-gray-500 text-xs sm:text-sm">Montant Payé</p>
                ...
                <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 hover:shadow-md transition-shadow">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-gray-500 text-xs sm:text-sm">Montant Restant</p>
              ...
              <div className="bg-white rounded-xl shadow-sm overflow-hidden">
                <div className="px-4 sm:px-6 py-3 sm:py-4 border-b bg-gray-50">
                  <h2 className="text-base sm:text-lg font-semibold text-gray-800">📋 Factures récentes</h2>
                </div>
                <div className="overflow-x-auto">
                  <table className="min-w-full">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="px-4 sm:px-6 py-2 sm:py-3 text-left text-xs font-medium text-gray-500 uppercase">Client</th>
                        <th className="px-4 sm:px-6 py-2 sm:py-3 text-right text-xs font-medium text-gray-500 uppercase">Total</th>
                        <th className="px-4 sm:px-6 py-2 sm:py-3 text-right text-xs font-medium text-gray-500 uppercase">Payé</th>
                        <th className="px-4 sm:px-6 py-2 sm:py-3 text-right text-xs font-medium text-gray-500 uppercase">Reste</th>
                        <th className="px-4 sm:px-6 py-2 sm:py-3 text-center text-xs font-medium text-gray-500 uppercase">Statut</th>
                        <th className="px-4 sm:px-6 py-2 sm:py-3 text-center text-xs font-medium text-gray-500 uppercase">Date</th>
                      </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-100">
                      {statsLoading ? (
                        <tr>
                          <td colSpan={6} className="px-4 sm:px-6 py-6 sm:py-8 text-center text-gray-500">Chargement...</td>
                        </tr>
                      ) : stats && stats.recentInvoices && stats.recentInvoices.length > 0 ? (
                        stats.recentInvoices.map((invoice: any) => (
                          <tr key={invoice.id} className="hover:bg-gray-50">
                            <td className="px-4 sm:px-6 py-2 sm:py-3 text-xs sm:text-sm text-gray-900">{invoice.client?.nom}</td>
                            <td className="px-4 sm:px-6 py-2 sm:py-3 text-xs sm:text-sm text-gray-900 text-right">{formatFCFA(invoice.total)}</td>
                            <td className="px-4 sm:px-6 py-2 sm:py-3 text-xs sm:text-sm text-green-600 text-right">{formatFCFA(invoice.montant_paye)}</td>
                            <td className="px-4 sm:px-6 py-2 sm:py-3 text-xs sm:text-sm text-red-600 text-right">{formatFCFA(invoice.reste_a_payer)}</td>
                            ...
                            <td className="px-4 sm:px-6 py-2 sm:py-3 text-xs sm:text-sm text-gray-500 text-center">
                        ...
                        <tr>
                          <td colSpan={6} className="px-4 sm:px-6 py-6 sm:py-8 text-center text-gray-500">Aucune facture trouvée</td>
                        </tr>
                      )}
                    </tbody>
                  </table>
